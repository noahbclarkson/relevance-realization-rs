...
fn calculate_agent_tasks(
    ...
    for (mut agent, mut agent_transform) in query.iter_mut() {
        ...
        if agent.task_history.is_empty() {
            ...
            // Essentially, we want to explore if the agent is in a good state
            // We want to exploit if the agent is in a bad state
            ...
            if exploration > exploitation {
                ...
                exploit(&mut agent, &time, &tiles, &mut agent_transform, &mut rng)
            }
        }
    }
}
...
fn exploit(agent: &mut Agent, time: &Res<Time>, tiles: &Res<Tiles>, mut agent_transform: &mut Transform, mut rng: &mut ThreadRng) {
    ...
    if let Some(task) = task {
        ...
        agent.exploit(&tiles, &mut agent_transform);
        agent.exploration_exploitation_state = ExploringExploitingState::Exploiting;
        agent.latest_task = lowest_value_task_type;
    } else {
        explore(agent, &time, &tiles, &mut agent_transform, &mut rng);
    }
}
